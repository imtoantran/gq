...
if($_POST['control']=='save_new'){
	echo "<script>window.location='spmoi.php?install=1';</script>";
}
?>
...
<div style="position:relative;margin:50px 0 0 20px; float:left">
	<div style="margin-top:-20px">
    		<div class="style_hover" onclick="window.location='quanlysp.php'" style="background:rgba(144,255,0,0.8); height:33px; width:110px; color:#fff; padding:8px 0 0 20px; float:left">GIÀY</div>
			<div class="style_hover" onclick="window.location='quanlypk.php'" style="background:#e34929; height:33px; width:110px; color:#fff; padding:8px 0 0 20px; margin-left:10px;float:left">PHỤ KIỆN</div>
            <div class="clr"></div>
    </div>
    <script>
		
        function short(name){
			window.location='quanlysp.php?short='+name+'&ASC='+<?php 
    ...
    <div style="margin-top:15px">
        <div>
            ...
            <div id="down" class="colum" style="height:120px;margin:32px 0 0 300px; padding-top:5px; position:relative; z-index:10">
                	<?php
                	$sql8 = "SELECT * FROM `muc` ORDER BY `id` ASC";
					$result8 = mysql_query($sql8, $connection);
					while($row8= mysql_fetch_array($result8)){ ?>
                    <li style="cursor:pointer; padding-left:5px; font-size:11px" 
	                onclick="window.location='quanlysp.php?mucloai_choice=<?php echo $row8['id'] ?>&#menu_qlsp'" onmousemove="style.color='#87a310'" onmouseout="style.color='#fff'">    
            ...
            <div class="colum">SIZE SẢN PHẨM</div>
        ...
        $sql = "SELECT * FROM `sp` ".$mucloai_choice." ORDER BY `$short` $ASC ";
        $result = mysql_query($sql, $connection);
        ...
        {?>
            ...
            <div style="margin-top:2px;">
            	<div class="colum2">
                	...
                	<img src="../images/product/shoes/<?php echo $row['id'] ?>_1.jpg" alt="" style="margin:15px 23px 0 23px" />
                </div>
            	<div class="colum2"><?php echo $row['ma'] ?></div>
            	<div class="colum2"><?php   $ID4 = $row['mucloai'];
											$sql3 = "SELECT * FROM `muc` WHERE `id` = $ID4";
            	...
            	<div class="colum2"><?php echo $row['size'] ?></div>
				<div class="colum" style="background:none;">
                    <div class="quanlytintuc_edit style_hover" style="margin-left:4px; background:#fff; color:#000" onclick="window.location='spmoi.php?item=<?php echo $row['id'] ?>'">EDIT</div>
