...
function install($code_mau){
		...
		$row = mysql_fetch_array(mysql_query("SELECT MAX(id) FROM `sp`"));
		$max_id=$row['MAX(id)']+1;
		...
		$s = "INSERT INTO `sp` (`id` ,`ten` ,`ma` ,`mucloai` ,`dongia` ,`mau` ,`size` ,`mota` ,`cachdung` ,`giaonhan` ,`hinhanh` ,`date`)";		
		...
		if(mysql_query($s)){
			if($_POST['control']=='save_new') echo "<script>window.location='spmoi.php?install=1';</script>";		
			else if($_POST['install']=='1') echo "<script>window.location='spmoi.php?item=".$max_id."';</script>";}
		}
function save($code_mau){
		...
		$size=trim($size);
		$n=strlen($size); $size=substr($size,0,$n-3);
		$query = "UPDATE `sp` SET `ten` = '%s',`ma` = '%s',`mucloai` = '%s',`dongia` = '%s',`mau` = '%s',`size` = '%s',`mota` = '%s',`cachdung` = '%s',`giaonhan` = '%s',`hinhanh` = '%s' WHERE `sp`.`id` ='%s'";
		$query = sprintf($query
			,mysql_real_escape_string($_POST['ten'])
			,mysql_real_escape_string($_POST['ma'])
			,mysql_real_escape_string($_POST['mucloai'])
		...
		if($_POST['control']=='save_new') echo "<script>window.location='spmoi.php?install=1';</script>";
		else if($_POST['control']=='save_close') echo "<script>window.location='quanlysp.php';</script>";
		else echo "<script>window.location='spmoi.php?item=".$_POST['item']."';</script>";
...
<div style="margin:70px 0 0 40%; position:relative; color:#fff">
	...
	<div style="margin-top:30px; background:#fff; width:471px">
			...
			<div style="margin-left:0" class="control_admin style_hover" 
            	onclick="save_data();document.form_quoc.control.value='save'; document.form_quoc.submit()">SAVE</div>
			<div class="control_admin style_hover" 
            	onclick="save_data();document.form_quoc.control.value='save_close'; document.form_quoc.submit()">SAVE & CLOSE</div>
			<div class="control_admin style_hover" 
            	onclick="save_data();document.form_quoc.control.value='save_new'; document.form_quoc.submit()">SAVE & NEW</div>
			<div class="control_admin style_hover"
            	 onclick="document.location='<?php	if($_GET['item']==NULL)echo 'index.php'; else echo 'quanlysp.php';?>'">CANCEL</div>
			<div class="clr"></div>
		</div>
</div>
<form method="post" name="form_quoc" id="form_quoc" action="" enctype="multipart/form-data">
    ...
    <input type="hidden" name="mucloai" value="<?php if($_GET['item']!=NULL) echo $item['mucloai']; else echo '7' ?>" />
...
<div style="position:relative;margin:20px 0 0 20px; float:left">
		<div>
			<div class="style_hover" onclick="window.location='spmoi.php'" style="background:rgba(144,255,0,0.8); height:33px; width:110px; color:#fff; padding:8px 0 0 20px; float:left">GIÀY</div>
			<div class="style_hover" onclick="window.location='pkmoi.php'" style="background:#e34929; height:33px; width:110px; color:#fff; padding:8px 0 0 20px; margin-left:10px;float:left">PHỤ KIỆN</div>
            <div class="clr"></div>
		</div>
		...
		<div style="margin-top:15px">
        	<div style="position:absolute; height:58px; width:280px; background:rgba(146,146,146,0.7);margin:-12px 0 0 450px;padding:2px 0 0 10px;">
            	<?php 
					$mucloai=array();
					$sql4 = "SELECT * FROM `muc` ORDER BY `id` ASC LIMIT 0,6 ";
			...
			<input name="mucloai_show" type="text" disabled="disabled"  style="float:left; background:#fff; height:33px; width:360px; border:none; color:#000;padding:0px 0 0 15px" value="<?php if($_GET['item']!=NULL) { if($item['mucloai']<7) {$idmuc=$item['mucloai']; echo $mucloai[$idmuc]['mucloai'];} else echo 'GIÀY TĂNG CHIỀU CAO';} else echo 'GIÀY TĂNG CHIỀU CAO'?>"  />
			<div class="clr"></div>
		</div>
		<div style="margin-top:15px">
			<div style="float:left; background:#e34929; height:33px; width:120px; color:#fff; padding:8px 0 0 20px">Đơn giá</div>
			<input name="dongia" type="text"  style="float:left; background:#fff; height:33px; width:360px; border:none; color:#000;padding:0px 0 0 15px" value="<?php if($_GET['item']!=NULL) echo $dongia=str_replace(',','.',number_format($item['dongia'],0))?>"   />
			<div class="clr"></div>
		</div>
		<div style="margin-top:15px">
		...
		<div style="margin-top:15px">
			<div style="float:left; background:#e34929; height:33px; width:120px; color:#fff; padding:8px 0 0 20px">Chọn size</div>
            <div style="width:190px; position:absolute; margin:-1px 0 0 120px;"><?php
            		$size=array("35","36","37","38","39","40","41","42");
					foreach($size as $value_size){	
					?>
                    <div class="box_color" style="background:rgba(255,255,255,1); margin-left:7px; margin-top:1px;"><?php echo $value_size ?></div>
                ...
                <?php
					...
					?>
                    <div class="box_color" onclick="set_on_off('onoff_<?php $u=$key+16; echo $u?>')" style="background:rgba(255,255,255,1); margin-left:7px; margin-top:1px;"><div id="onoff_<?php $u=$key+16; echo $u?>" style="margin:5px; display:none; background:#929292; height:5px; width:5px;"></div></div>
                ...
                <?php }?>
                        <script>
							...
							function save_data(){
									...
									document.form_quoc.data_onoff2.value=data2;
							}
							...
							if($_GET['item'] && $item['size']){		
								function check_size($valuea){
									$valuea=trim($valuea);
									if($valuea==35)return 16;
									if($valuea==36)return 17;
									if($valuea==37)return 18;
									if($valuea==38)return 19;
									if($valuea==39)return 20;
									if($valuea==40)return 21;
									if($valuea==41)return 22;
									if($valuea==42)return 23;

								}						
								$masize=array();
								$masize=explode(',',$item['size']);
								foreach($masize as $array_masize){?>onoff_[<?php echo check_size($array_masize); ?>]=0;<?php }   // Set off các mục cần chọn.
